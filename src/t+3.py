...
def main():
    ...
    df_merged[['impuestos_totales','tur_mensual','remesas']]=(df_merged[['impuestos_totales','tur_mensual','remesas']]).diff(5)


    df_merged[cartera_vars]=((df_merged[cartera_vars])).diff(5)
    df_merged['imae_cambio']=(df_merged[['imae']]).diff(12)
    df_merged['imae_cambio_target']=(df_merged['imae_cambio']).shift(-3)
    df_merged['imae_cambio_11']=(df_merged[['imae']]).diff(9)
    df_merged['imae_shifted_11']=df_merged['imae'].shift(9)
    ...
    results['Fecha']=results['Fecha'].shift(-3)
    results['imae_cambio_real']=results['imae_cambio_real'].shift(-3)
    results['imae_shifted_11']=results['imae_shifted_11']
    results['Fecha'].iloc[-1]=results.iloc[-4]['Fecha']+(results.iloc[-5]['Fecha']-results.iloc[-8]['Fecha'])
    results['Fecha'].iloc[-2]=results.iloc[-4]['Fecha']+(results.iloc[-5]['Fecha']-results.iloc[-7]['Fecha'])
    results['Fecha'].iloc[-3]=results.iloc[-4]['Fecha']+(results.iloc[-5]['Fecha']-results.iloc[-6]['Fecha'])
    ...
    plt.title('Predicción del Indicador Mensual de Actividad Económica (IMAE) t+3 - Diferencia')
    ...
    plt.title('Predicción del Indicador Mensual de Actividad Económica (IMAE) t+3 - Nivel')
    plt.grid(True)
    plt.show()
    ...
    plt.grid(True)
    plt.title('Residuos de la diferencia t+3')
    ...
    #Aquí ploteamos el density forecast para t+3
    stats_text = (
        f'Promedio: {mean:.2f}\n'
        f'Varianza: {second_moment:.2f}\n'
        f'Desviación estándar: {math.sqrt(second_moment):.2f}\n'
    ...
    plt.title('Density Forecast t+3')
    plt.grid(True)
    plt.show()
