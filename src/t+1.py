...
#Esta función sirve para formatear la fecha
def get_date(x):
    # print(x.values)
...
def main():
    ...
    impuestos = impuestos.rename(columns={'fecha': 'Fecha'})
    ...
    imae = imae.rename(columns={'fecha': 'Fecha'})
    ...
    remesas = remesas.rename(columns={'fecha': 'Fecha'})
    ...
    tipm_interbancaria = tipm_interbancaria.rename(columns={'fecha': 'Fecha'})
    ...
    print(tipm_interbancaria.columns)
    ...
    IPC = IPC.rename(columns={'fecha': 'Fecha'})
    ...
    tipm_prom_p = tipm_prom_p.rename(columns={'fecha': 'Fecha'})
    ...
    tiam_prom_p = tiam_prom_p.rename(columns={'fecha': 'Fecha'})
    ...
    df_merged['imae_cambio_target']=(df_merged['imae_cambio']).shift(-1)
    df_merged['imae_cambio_11']=(df_merged[['imae']]).diff(11)

    df_merged['imae_shifted_11']=df_merged['imae'].shift(11)
    ...
    results['Fecha']=results['Fecha'].shift(-1)
    results['imae_cambio_real']=results['imae_cambio_real'].shift(-1)
    results['imae_shifted_11']=results['imae_shifted_11']
    results['Fecha'].iloc[-1]=results.iloc[-2]['Fecha']+(results.iloc[-2]['Fecha']-results.iloc[-3]['Fecha'])
    ...
    plt.title('Predicción del Indicador Mensual de Actividad Económica (IMAE) t+1 - Diferencia')
    ...
    plt.title('Predicción del Indicador Mensual de Actividad Económica (IMAE) t+1 - Nivel')
    plt.grid(True)
    plt.show()
    ...
    plt.title('Residuos de la diferencia t+1')
    ...
    #Aquí ploteamos el density forecast para t+1
    stats_text = (
        f'Promedio: {mean:.2f}\n'
        f'Varianza: {second_moment:.2f}\n'
        f'Desviación estándar: {math.sqrt(second_moment):.2f}\n'
    ...
    plt.title('Density Forecast t+1')
    plt.grid(True)
    plt.show()
